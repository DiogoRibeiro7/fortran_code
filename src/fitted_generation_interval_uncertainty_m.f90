module fitted_generation_interval_uncertainty_m
    ...
    public :: generation_interval_bootstrap_summary, fitted_rt_monte_carlo_summary
    public :: propagate_fitted_generation_interval_uncertainty_rt

    type :: generation_interval_bootstrap_summary
        ...
        logical :: valid = .false.
    end type generation_interval_bootstrap_summary

    type :: fitted_rt_monte_carlo_summary
        real(real64) :: mean = 0.0_real64
        real(real64) :: lower = 0.0_real64
        real(real64) :: upper = 0.0_real64
        integer :: n_draws = 0
        logical :: valid = .false.
    end type fitted_rt_monte_carlo_summary

contains

    subroutine propagate_fitted_generation_interval_uncertainty_rt(pair_intervals, reported_counts, ages, &
            delay, max_lag, nowcast_prior_shape, nowcast_prior_rate, rt_window_size, rt_prior_shape, &
            rt_prior_rate, credible_level, seed, n_draws, rt_summaries, bootstrap_summary)
        real(real64), intent(in) :: pair_intervals(:), reported_counts(:)
        ...
        type(fitted_rt_monte_carlo_summary), intent(out) :: rt_summaries(:)
        type(generation_interval_bootstrap_summary), intent(out) :: bootstrap_summary
        ...
        real(real64), allocatable :: bootstrap_intervals(:), weights(:), complete_incidence(:)
        ...
        n_pairs = size(pair_intervals)
        if (n_pairs < 2) error stop "at least two transmission-pair intervals are required"
        if (any(pair_intervals <= 0.0_real64)) error stop "transmission-pair intervals must be positive"
        if (n_days < 1 .or. size(ages) /= n_days .or. size(rt_summaries) /= n_days) then
            error stop "reported counts, ages, and Rt summaries must have matching non-empty lengths"
        end if
        if (any(reported_counts < 0.0_real64) .or. any(ages < 0)) then
            error stop "reported counts and ages must be non-negative"
        end if
        ...
        allocate(bootstrap_intervals(n_pairs), weights(max_lag), valid_counts(n_days))
        allocate(rt_draws(n_days, n_draws), shape_draws(n_draws), rate_draws(n_draws), &
            mean_draws(n_draws), tail_draws(n_draws))

        rt_summaries = fitted_rt_monte_carlo_summary()
        bootstrap_summary = generation_interval_bootstrap_summary()
        ...
        do draw_idx = 1, n_draws
            do day_idx = 1, n_pairs
                ...
                bootstrap_intervals(day_idx) = pair_intervals(source_idx)
            end do

            call fit_gamma_generation_interval(bootstrap_intervals, fit)
            if (.not. fit%converged) cycle
            call discretize_gamma_generation_interval(fit, max_lag, weights, tail_probability)
        ...
        do day_idx = 1, n_days
            if (valid_counts(day_idx) < 2) cycle
            call summarize_rt_draws(rt_draws(day_idx, :valid_counts(day_idx)), credible_level, &
                rt_summaries(day_idx))
        end do
    end subroutine propagate_fitted_generation_interval_uncertainty_rt
    ...
    subroutine summarize_bootstrap(shape_draws, rate_draws, mean_draws, tail_draws, summary)
        ...
        type(generation_interval_bootstrap_summary), intent(inout) :: summary
    ...
    subroutine summarize_rt_draws(draws, credible_level, summary)
        real(real64), intent(in) :: draws(:), credible_level
        type(fitted_rt_monte_carlo_summary), intent(out) :: summary
        ...
        summary%valid = .true.
    end subroutine summarize_rt_draws
    ...
    pure real(real64) function sample_sd(values) result(value)
        ...
        value = sqrt(sum((values - mean_value)**2) / real(size(values) - 1, real64))
...
end module fitted_generation_interval_uncertainty_m
